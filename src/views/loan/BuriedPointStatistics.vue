...
<script>
import { appStepList, appStepPercent } from '@/api/apiLoanStatistics'
import { removeNoUseKeys } from '@/utils'
export default {
  name: 'BuriedPointStatistics',
  data() {
    return {
      total: 10,
      tableData: [],
  ...
  methods: {
    ...
    async getTableList() {
      const res = await appStepList(removeNoUseKeys(this.queryInfo))
      if (res.code === 200) {
        this.total = res.data.total
        this.tableData = res.data.records
      }

      const percentRes = await appStepPercent(removeNoUseKeys(this.queryInfo))
