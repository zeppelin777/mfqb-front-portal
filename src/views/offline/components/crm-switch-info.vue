    ...
    <el-table border stripe :data="tableData">
      ...
      <el-table-column label="渠道类型" align="center">
        <template slot-scope="scope">
            <el-tag v-if="scope.row.channelType == 1">全流程</el-tag>
            <el-tag v-if="scope.row.channelType == 2">半流程</el-tag>
            <el-tag v-if="scope.row.channelType == 3">UV</el-tag>
            <el-tag v-if="scope.row.channelType == 4">其他</el-tag>
          </template>
      </el-table-column>
...
<script>
import {
  crmPlatformSwitchList,
  changeCrmSwitchInfo,
  updateSwitchByPlatId
} from '@/api/crm'

export default {
  name: 'CrmSwitchDialog',
  props: {
    isVisible: {
      type: Boolean,
      default: false
  ...
  methods: {
    async getTableList() {
      this.queryInfo.crmPlatformId = this.crmPlatformId
      const res = await crmPlatformSwitchList(this.queryInfo)
      if (res.code === 200) {
    ...
    async changeSwitchFlag(e, row) {
      ...
      const res = await changeCrmSwitchInfo(params)
      if (res.code === 200) {
    ...
    closeAll(e) {
      ...
      this.$confirm(text)
        .then(async () => {
          let params = {
            id: this.crmPlatformId,
            switchFlag: e
          }

          const res = await updateSwitchByPlatId(params)
          if (res.code === 200) {
