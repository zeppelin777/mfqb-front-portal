<template>
  <div class="push-container">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>渠道访问日志</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card class="opt-box">
      <el-form inline>
        ...
        <el-form-item label="ip：">
          <el-input v-model="queryInfo.queryIp" placeholder="请输入ip" clearable />
        </el-form-item>
        <br/>
        <el-form-item label="进件状态：">
          <el-select
            v-model="queryInfo.loanFlag"
            clearable
            placeholder="请选择">
            <el-option
        ...
        <el-form-item label="注册状态：">
          <el-select
            v-model="queryInfo.registerFlag"
            clearable
            placeholder="请选择">
            <el-option
              v-for="item in stateOptions"
      ...
      <el-table border stripe :data="tableData">
        <el-table-column prop="id" label="访问流水号" align="center" />
        ...
        <el-table-column prop="device" label="ip所在地" align="center">
          <template slot-scope="scope">
            {{ scope.row.province }}-{{ scope.row.city }}
          </template>
        </el-table-column>
        <el-table-column prop="device" label="设备" align="center" />
        <el-table-column prop="loanFlag" label="是否注册" align="center">
          <template slot-scope="scope">
            {{ scope.row.registerFlag ? '是' : '否' }}
          </template>
        </el-table-column>
        <el-table-column prop="loanFlag" label="是否进件" align="center">
          <template slot-scope="scope">
            {{ scope.row.loanFlag ? '是' : '否' }}
          </template>
        </el-table-column>
        <el-table-column prop="ctime" label="访问时间" align="center"></el-table-column>
      </el-table>
      <el-pagination
...
<script>
import { channelLogList } from '@/api/log'
export default {
  name: 'PushStatistics',
  data() {
    return {
  ...
  methods: {
    async getTableList() {
      const res = await channelLogList(this.queryInfo)
      if (res.code === 200) {
